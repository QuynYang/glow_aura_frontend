...
export const SkinQuizResultPage = () => {
  ...
  const skinTypeCode = location.state?.skinType as string | undefined;
  const quizResult = location.state?.quizResult as Record<string, any> | undefined;
    ...
    const fetchResultData = async () => {
      try {
        ...
        const [details, products] = await Promise.all([
          skinQuizService.getSkinTypeDetails(skinTypeCode).catch(() => ({})),
  ...
  const advices =
    (skinDetails.skincareTips?.length > 0 ? skinDetails.skincareTips : null) ||
    (quizResult?.skincareTips && quizResult.skincareTips.length > 0 ? quizResult.skincareTips : null) ||
    skinDetails.advices ||
    [
  ...
  const sidebarItems =
    recommendedProducts.length > 0
      ? recommendedProducts.slice(0, 3).map((p: any) => ({
          id: p.id,
          name: p.name,
          brand: p.brand || 'GLOW AURA',
          price: `${(p.discountedPrice ?? p.price).toLocaleString('vi-VN')}đ`,
          oldPrice: p.discountedPrice && p.price ? `${p.price.toLocaleString('vi-VN')}đ` : '',
        ...
        <div className="container mx-auto px-4 py-12 animate-in fade-in duration-700 max-w-6xl">
            ...
            <div className="flex flex-col lg:flex-row gap-8 mb-16">
                ...
                <div className="w-full lg:w-2/3 space-y-6">
                    ...
                    <div>
                        <h1 className="text-3xl md:text-4xl font-black text-[#3D021E] mb-2 uppercase tracking-wide">
                            KẾT QUẢ:{' '}
                            {quizResult?.skinTypeName || skinDetails.name || skinDetails.title || skinTypeCode || 'Đang xử lý'}
                        </h1>
                    ...
                    <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
                        <div className="flex flex-col items-start gap-6">
                            <div className="inline-flex flex-col items-center justify-center border border-gray-100 bg-gray-50 rounded-2xl p-4 min-w-[120px]">
                                <span className="text-4xl font-black text-[#3D021E] mb-1">
                                  {quizResult?.confidencePercent ?? skinDetails.confidencePercent ?? 98}%
                                </span>
                            ...
                            <div className="border-l-4 border-gray-200 pl-5 py-1">
                                <p className="text-gray-700 leading-relaxed font-medium">
                                    {quizResult?.aiSummary ||
                                      ...
                                      skinDetails.summary ||
                                      'Làn da của bạn cần được chăm sóc đặc biệt để duy trì sự cân bằng tự nhiên và khỏe mạnh từ sâu bên trong.'}
                                </p>
                            </div>
                ...
                <div className="w-full lg:w-1/3 space-y-6">
                    ...
                    <div className="bg-[#3D021E] rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
                        ...
                        <Link to="/best-sellers" className="w-full bg-white text-[#3D021E] font-bold py-4 rounded-xl hover:bg-gray-100 transition-colors shadow-lg relative z-10 block text-center">
                            Mua sắm ngay
                        </Link>
                        <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-pink-500 opacity-20 rounded-full blur-3xl"></div>
                    </div>

                    {/* Khối Dành riêng cho bạn */}
            ...
            <div className="border-t border-gray-200 pt-16">
                <div className="flex justify-between items-end mb-10">
                    ...
                    <Link to="/best-sellers" className="text-sm font-bold text-gray-500 hover:text-[#3D021E] underline underline-offset-4 transition-colors">Xem tất cả sản phẩm</Link>
                </div>
